...
import { Plus, Edit3, Trash2, Check, X, Tag as TagIcon } from 'lucide-react';
...
export default function TagsManager({ onTagsChange }: TagsManagerProps) {
  ...
  const handleDelete = (id: string) => {
    if (confirm('Удалить тег? Он будет удалён у всех паломников.')) {
      deleteTag(id);
      refreshTags();
    }
  };
    ...
    <div className="space-y-4">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3">
          ...
          <div>
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              />
              <span className="text-xs text-gray-400 font-mono">{newTag.color}</span>
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={() => { setShowAddForm(false); setNewTag({ name: '', color: '#3B82F6' }); }} className="px-4 py-2 border rounded-lg text-sm hover:bg-gray-50">
              Отмена
            </button>
          </div>
        </div>
      ...
      {editingTag && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 space-y-3">
          ...
          <div className="flex gap-2">
            <button onClick={handleUpdate} className="px-4 py-2 bg-amber-600 text-white rounded-lg text-sm hover:bg-amber-700">
              Сохранить
            </button>
            <button onClick={() => setEditingTag(null)} className="px-4 py-2 border rounded-lg text-sm hover:bg-gray-50">
              Отмена
            </button>
          </div>
        </div>
      ...
      {tags.length === 0 ? (
        <div className="text-center py-8 text-gray-400">
          ...
          <p className="text-xs mt-1">Создайте теги для группировки паломников</p>
        </div>
      ) : (
        <div className="flex flex-wrap gap-2">
          {tags.map(tag => (
      ...
      <div className="bg-gray-50 rounded-lg p-3 text-xs text-gray-500">
        💡 <strong>Подсказка:</strong> Теги можно назначать паломникам в карточке. 
        Они отображаются в таблице и помогают быстро фильтровать записи.
      </div>
    </div>
  );
}
